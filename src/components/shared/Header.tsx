        ...
        <>
            ...
            >
                <div className="max-w-7xl mx-auto flex justify-between items-center gap-4">
                    ...
                    {/* Navigation Desktop */}
                    <nav className="hidden md:flex items-center gap-1 bg-stone-100/90 p-1.5 rounded-full border border-stone-200/80">
                        {NAV_LINKS.map((link) => (
                            <Link
                                key={link.id}
                                href={link.href}
                                className="px-4 py-1.5 text-sm font-medium text-stone-700 hover:text-orange-700 rounded-full hover:bg-white transition-all duration-200"
                            >
                                {link.label}
                            </Link>
                        ))}
            ...
            {isMenuOpen && (
                <div className="fixed inset-0 z-50 md:hidden flex justify-end bg-stone-950/40 backdrop-blur-xs animate-in fade-in duration-200">
                    <div
                                ...
                                {NAV_LINKS.map((link) => (
                                    <li key={link.id}>
                                        <Link
                                            href={link.href}
                                            onClick={() => setIsMenuOpen(false)}
                                            className="block px-4 py-3 text-base font-medium text-stone-800 hover:text-orange-700 hover:bg-orange-50/70 rounded-xl transition-colors"
                                        >
                                            {link.label}
                                        </Link>
                                    </li>
